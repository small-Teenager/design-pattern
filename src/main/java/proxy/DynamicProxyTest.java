 ...
 * @date: 2021/11/9 18:27
 */
public class DynamicProxyTest {
    /**
     * 虽然相对于静态代理，动态代理大大减少了我们的开发任务，同时减少了对业务接口的依赖，
     * 降低了耦合度。但是还是有一点点小小的遗憾之处，
     * 那就是它始终无法摆脱仅支持interface代理的桎梏，因为它的设计注定了这个遗憾。
     * 回想一下那些动态生成的代理类的继承关系图，它们已经注定有一个共同的父类叫Proxy。
     * Java的继承机制注定了这些动态代理类们无法实现对class的动态代理，
     * 原因是多继承在Java中本质上就行不通。有很多条理由，
     * 人们可以否定对 class代理的必要性，但是同样有一些理由，
     * 相信支持class动态代理会更美好。接口和类的划分，本就不是很明显，
     * 只是到了Java中才变得如此的细化。如果只从方法的声明及是否被定义来考量，
    ...
    public static void main(String[] args) {
        ...
        Internet proxyInstance = (Internet) Proxy.newProxyInstance(Internet.class.getClassLoader(), new
                Class[]{Internet.class}, new DynamicProxyHandler(internet));
        proxyInstance.surfing();
    }
}
